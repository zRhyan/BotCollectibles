...
async def add_card(message: types.Message):
    ...
    if not message.reply_to_message.caption:
        await message.reply(
            "❗ **Erro:** A mensagem respondida deve conter uma legenda com o nome e a raridade do card.", 
            parse_mode=ParseMode.MARKDOWN
        )
        return
    ...
    try:
        ...
        await message.reply(
            "⚠️ **Formato inválido!** A legenda deve estar no formato: `Nome do Card | Raridade`.", 
            parse_mode=ParseMode.MARKDOWN
        )
        return
